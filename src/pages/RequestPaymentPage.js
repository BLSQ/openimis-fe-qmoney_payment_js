...
function RequestPaymentPage(props) {
  const { location: { state: { classes, familyLocation, policyUuid = null, maxPaymentAmount = 0, requestPaymentFn = requestPayment } = {} } = {} } = props;
  ...
  const onSuccess = useCallback((response) => {
    setFailed(false);
    setLoading(false);
    const qmoneyPaymentUuid = response.requestQmoneyPayment.qmoneyPayment.uuid;
    history.push({
      pathname: `/${ROUTE_TO_CONFIRM_PAYMENT_PAGE}`,
      state: { classes: classes, qmoneyPaymentUuid: qmoneyPaymentUuid, familyLocation: familyLocation }
    });
  }, [history]);
  ...
  const validateAndSetQmoneyWallet = useCallback((event) => {
  ...
  const validateAndSetPaymentAmount = useCallback((event) => {
    ...
    if (isNaN(amount) || amount === '') {
      ...
      setPaymentAmountError(true);
      setPaymentAmountErrorMessage('Please provide a positive amount to pay.');
    } else if (amount > maxPaymentAmount) {
      setPaymentAmountError(true);
      setPaymentAmountErrorMessage(`Please provide an amount less than the total amount to pay ${maxPaymentAmount}.`);
    } else {
  ...
  const onRequestPayment = useCallback((event) => {
    event.preventDefault();

    setFailed(false);
    setLoading(true);

    requestPaymentFn(policyUuid, paymentAmount, qmoneyWallet, onSuccess, onError);
  }, [qmoneyWallet, policyUuid, paymentAmount, requestPaymentFn]);

  const onCancelRequestPayment = useCallback((event) => {
    event.preventDefault();
    history.goBack();
  }, [history]);
          ...
          <form onSubmit={onRequestPayment}>
            <Grid container direction="row">
              <FormControl>
                ...
                <Input id="qmoney-wallet" aria-describedby="qmoney-wallet-helper-text" autoFocus value={qmoneyWallet} onChange={validateAndSetQmoneyWallet} error={qmoneyWalletError} required/>
                <FormHelperText id="qmoney-wallet-helper-text">{qmoneyWalletError && 'Please provide the QMoney wallet id of the payer.'}</FormHelperText>
              </FormControl>
            </Grid>
            <Grid container direction="row">
              <FormControl>
                <InputLabel htmlFor="payment-amount"></InputLabel>
                <Input type="number" id="payment-amount" aria-describedby="payment-amount-helper-text" value={paymentAmount} onChange={validateAndSetPaymentAmount} error={paymentAmountError} required/>
                <FormHelperText id="payment-amount-helper-text">{paymentAmountErrorMessage}</FormHelperText>
              </FormControl>
            </Grid>
            <Grid container direction="row">
              <ButtonGroup aria-label="Request payment buttons">
                <Button disabled={qmoneyWalletError || paymentAmountError || loading} label="Request" type="Submit">Request</Button>
