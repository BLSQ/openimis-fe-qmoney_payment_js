...
it("renders an error when none policy provided", () => {
  ...
  expect(screen.getByRole("heading").textContent).toBe(
    "Please provide a Policy to request a payment."
  );
});
...
it("renders a form when passed policy", () => {
  ...
  expect(screen.getByRole("heading").textContent).toBe(
    "Request a mobile payment by QMoney"
  );
  expect(
    screen.getByText("Please provide the QMoney wallet id of the payer.")
  ).toBeInTheDocument();
  ...
  act(() => {
    render(
      <TestRouter
        history={history}
        routes={routes}
        initialPath={`/${ROUTE_TO_REQUEST_PAYMENT_PAGE}`}
      />
    );
  });
